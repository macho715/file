...
| [UPGRADE_LLM_TO_CURSOR_CODEX_REPLACEMENT.md](UPGRADE_LLM_TO_CURSOR_CODEX_REPLACEMENT.md) | 로컬 LLM 없이 Cursor 서브에이전트·스킬·Codex 스킬로 대체하는 방법 |
| [UPGRADE_LLM_DEEP_SYNTH_REPORT.md](UPGRADE_LLM_DEEP_SYNTH_REPORT.md) | LLM 대체 Best 3 Deep Report (--no-llm, 스킬 재정의, NO_LOCAL_LLM 워크플로) |
| [UPGRADE_LLM_VERIFIER_REPORT.md](UPGRADE_LLM_VERIFIER_REPORT.md) | LLM 대체 업그레이드 Verifier (Non-Negotiables·Apply Gates·Rollback) |
| [UPGRADE_DOC_AUDIT_REPORT.md](UPGRADE_DOC_AUDIT_REPORT.md) | 문서 우선 진단 (Current State Snapshot, evidence_paths, Pain/Quick wins) |
| [PLAN_LLM_REPLACEMENT.md](PLAN_LLM_REPLACEMENT.md) | LLM 대체 실행 플랜 (A~K+ㅋ, PR 6개, Evidence, Risk Register) |
...
- 미분류 문서(`.pdf/.docx/.xlsx`)는 `quarantine`으로 이동됩니다.
- ledger reason은 `no_llm_ambiguous_doc`로 기록됩니다.
- triage 절차는 `docs/NO_LOCAL_LLM_WORKFLOW.md`를 따릅니다.

추가 문서:
- [docs/NO_LOCAL_LLM_WORKFLOW.md](docs/NO_LOCAL_LLM_WORKFLOW.md)
