...
| `watchers:sweepAll` | 특정 폴더/전체 수동 스캔 | `id?: number` | `{ scannedCount }` |
...
sweepWatchers: (id?: number) =>
  ipcRenderer.invoke('watchers:sweepAll', id),
...
### 마이그레이션 로직 (services/migration.ts)

```typescript
export async function runLegacyMigration(db, autosortPaths): Promise<void>
...
// 실행 조건:
//   settings 테이블에 'migration_legacy_v1_done' 키가 없을 때만 실행
//   완료 후 setSetting(db, 'migration_legacy_v1_done', 'true')
```
...
await runLegacyMigration(db, autosortPaths);  // ★ 추가
```
...
| `src/main/services/migration.ts` | **수정** | cache.json + ledger.jsonl → SQLite 마이그레이션 |
...
- 참고:
  - 현재 로컬에서 `node_modules` 미설치 상태면 `vitest/tsc/vite` 명령이 실행되지 않음
