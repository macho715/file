...
## 3) MVP 코드 “최소 패치” (autosortd_1py.py 기준)

autosortd_1py.py는 이미 rules.yaml/mapping.yaml을 사용하므로, 아래는 규칙/매핑 개념 참고용입니다. 코드 변경은 rules·mapping 수정으로 반영합니다.
...
python C:\_AUTOSORT\autosortd_1py.py --root C:\_AUTOSORT --watch "D:\당신_작업폴더" --llm "http://127.0.0.1:8080/v1" --sweep
```

(1회만 처리 시 `--sweep` 대신 `--once`)
...
3. **다음행동:** **RULE-20**은 autosortd_1py.py에서 rules.yaml 규칙으로 반영됩니다. `Temp`는 mapping.yaml의 doc_type_map에서 분리 가능합니다.
...
* `/redo step` : RULE-20을 **rules.yaml 규칙**으로 합쳐서 “붙여넣기 완성본” 생성
...
원하면 다음 턴에 **“autosortd_1py.py(rules.yaml) 기준 완성 규칙”**를 그대로 출력하겠습니다.
