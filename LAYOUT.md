...
file/
  ...
  dashboard.py              # ledger/cache 상황판 (--root, --refresh)
  autosort-triage-apply.py  # quarantine *.decision.json 적용 (이동·ledger)
...
**SSOT**: 런타임 폴더 트리는 `C:\_AUTOSORT` 또는 `--root`/`--base`로 지정한 경로 아래에 고정된다. 아래 구조가 단일 진실 원천(SSOT)이다.
...
**autosort-desktop (Electron)**  
- 빌드 결과: `autosort-desktop/dist/main/`(메인 프로세스), `autosort-desktop/dist/renderer/`(렌더러 UI). 규칙 파일은 앱 설정에서 루트 경로 지정 시 해당 `rules/`를 참조한다.
- 문서: `autosort-desktop/docs/` — PHASE2-PLAN.md, WORKLOG_BUILD_WINDOWS_2026-03.md(Windows npm run build 안정화), GATE3_EVIDENCE_TEMPLATE.md(참고용 검증 체크리스트) 등.

**브릿지/스크립트**  
- **autosort-triage-apply.py** (프로젝트 루트): quarantine 내 `*.decision.json`을 읽어 Cursor/Codex triage 결과를 적용(이동·ledger 기록). `--dry-run` 지원.
...
| autosort-desktop/docs/WORKLOG_BUILD_WINDOWS_2026-03.md | Windows npm run build 안정화 상세 이력 |
| autosort-desktop/docs/GATE3_EVIDENCE_TEMPLATE.md | Gate 3 검증 체크리스트 (참고: email_search PR1–PR9) |
