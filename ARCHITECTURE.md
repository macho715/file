...
**옵션: --no-llm**  
- 데몬에 `--no-llm` 지정 시 LLM warmup/호출 생략. 규칙으로 분류 불가한 문서(.pdf/.docx/.xlsx)는 즉시 **quarantine**으로 이동하고 ledger에 reason `no_llm_ambiguous_doc` 기록. (PLAN_LLM_REPLACEMENT, NO_LOCAL_LLM_WORKFLOW 참고.)

**Electron 빌드 (Windows)**  
- `npm run build` = build:main(tsc) + build:renderer(vite). Windows cmd에서 Node 경로/공백 이슈 회피를 위해 `package.json`에 `for %I in ("%ProgramFiles%\nodejs\node.exe") do @"%~sI"` 형태로 node 직접 호출 사용. 상세: autosort-desktop/docs/WORKLOG_BUILD_WINDOWS_2026-03.md.
...
**데이터 저장 구분**  
- **Python 데몬(autosortd_1py)**: `logs/ledger.jsonl`(모든 이동 기록), `cache/cache.json`(SHA256→결정 캐시).  
- **Electron desktop Phase2**: SQLite DB가 ledger·파일 인덱스·dedup_cache의 SSOT가 될 예정이며, 기존 ledger.jsonl·cache.json 임포트 지원.
...
| LLM | 없음 | 있음(선별). `--no-llm` 시 생략, 미분류 문서 → quarantine + ledger `no_llm_ambiguous_doc` |
| Staging | 없음 | 있음 |
...
## 15a. Electron 앱 (autosort-desktop)

| 역할 | 구성요소 | 설명 |
|------|----------|------|
| Main process | `dist/main/` | IPC, Watcher, DB 접근, 스케줄 기동 |
| Renderer | `dist/renderer/` | UI(대시보드, 설정, 파일 목록) |
| 분류·이동 | classifier, move-engine | 규칙 기반 분류 + 목적지 해석, 원자적 이동 |
| LLM | llm-client | PDF/DOCX/XLSX 등 선별 호출 (Python 데몬과 동일 정책) |

**현재 상태 vs Phase2**

| 영역 | 현재 (Phase 1) | Phase2 예정 |
|------|----------------|-------------|
| 분류 결과 | cache.json (Python), 앱은 미연동 | SQLite `files` 테이블 단일 SSOT |
| Ledger | ledger.jsonl (Python) | SQLite `ledger` 테이블 + 기존 jsonl 마이그레이션 |
| 중복 캐시 | cache.json | SQLite `dedup_cache` |
| Quarantine 워크플로우 | 버튼만 존재 | 승인/재분류/삭제 핸들러 연동 |
...
| **--no-llm** | LLM 호출 생략 | 규칙으로만 분류. 미분류 문서(PDF/DOCX/XLSX 등)는 quarantine으로 이동 + ledger에 `reason: no_llm_ambiguous_doc` 기록. 로컬 LLM 없이 사용 시 이 옵션 사용. |
