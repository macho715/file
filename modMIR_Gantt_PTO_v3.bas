...
Public Sub Export_Summary_Report()
    On Error GoTo EH
    Dim t0 As Double: t0 = Timer
    Dim rName As String: rName = "MIR_Report_" & Format(Now(), "YYYYMMDD_HHMMSS")

    Application.DisplayAlerts = False
    On Error Resume Next: ThisWorkbook.Sheets(rName).Delete: On Error GoTo EH
    Application.DisplayAlerts = True
    ...
    wsR.Cells(row, 1).Value = "MIR REACTOR REPAIR -- PTO STATUS REPORT"
    wsR.Cells(row, 1).Font.Bold = True
    wsR.Cells(row, 1).Font.Size = 13
    row = row + 1
    wsR.Cells(row, 1).Value = "Generated: " & Format(Now(), "yyyy-mm-dd hh:mm")
    row = row + 1
    wsR.Cells(row, 1).Value = "Module: modMIR_Gantt_PTO v3.0"
    row = row + 2
    ...
    If Not wsCL Is Nothing Then
        Dim clR As Long, openCnt As Long, confCnt As Long
        Dim clLast As Long: clLast = wsCL.Cells(wsCL.Rows.Count, 2).End(xlUp).Row
        For clR = 4 To clLast
            Dim st As String: st = UCase$(Trim$(CStr(wsCL.Cells(clR, 5).Value)))
            If st = "OPEN" Then openCnt = openCnt + 1
            If st = "CONFIRMED" Then confCnt = confCnt + 1
        Next clR
        wsR.Cells(row, 1).Value = "=== DSV CHECKLIST ==="
        wsR.Cells(row, 1).Font.Bold = True: row = row + 1
        wsR.Cells(row, 1).Value = "OPEN: " & openCnt & "   CONFIRMED: " & confCnt
        row = row + 2
    End If

    ' ── Scenario summaries ──
    Dim scenario As Variant
    For Each scenario In Array("BASE", "BEST", "WORST")
        Dim wsG As Worksheet
        On Error Resume Next
        Set wsG = ThisWorkbook.Sheets(SHEET_PREFIX & scenario)
        On Error GoTo EH
        If wsG Is Nothing Then GoTo NextScen

        wsR.Cells(row, 1).Value = "=== GANTT_" & scenario & " ==="
        wsR.Cells(row, 1).Font.Bold = True: row = row + 1

        Dim headers As Variant
        headers = Array("#", "Phase", "Task", "Start", "End", "Days", "Risk", "%Done")
        Dim h As Long
        For h = 0 To UBound(headers)
            wsR.Cells(row, h + 1).Value = headers(h)
            wsR.Cells(row, h + 1).Font.Bold = True
        Next h
        row = row + 1
        ...
        For gR = FIRST_DATA_ROW To gLast
            ...
            If IsNumeric(wsG.Cells(gR, COL_PCT).Value) Then
                pctVal = Format(CDbl(wsG.Cells(gR, COL_PCT).Value) * 100, "0") & "%"
            End If

            wsR.Cells(row, 1).Value = wsG.Cells(gR, COL_STEP).Value
            wsR.Cells(row, 2).Value = wsG.Cells(gR, COL_PHASE).Value
            wsR.Cells(row, 3).Value = wsG.Cells(gR, COL_TASK).Value
            wsR.Cells(row, 4).Value = Format(wsG.Cells(gR, COL_START).Value, "yyyy-mm-dd")
            wsR.Cells(row, 5).Value = Format(wsG.Cells(gR, COL_END).Value, "yyyy-mm-dd")
            wsR.Cells(row, 6).Value = wsG.Cells(gR, COL_DAYS).Value
            wsR.Cells(row, 7).Value = wsG.Cells(gR, COL_RISK).Value
            wsR.Cells(row, 8).Value = pctVal
            row = row + 1
NextGRow:
        Next gR
        row = row + 1
NextScen:
    ...
    LogMsg "Export_Summary_Report", rName & " created in " & Format(Timer - t0, "0.00") & "s"
    MsgBox "Report: " & rName & vbCrLf & "Elapsed: " & Format(Timer - t0, "0.00") & "s", _
...
Public Sub Variance_Report()
    On Error GoTo EH
    Dim rName As String: rName = "Variance_" & Format(Now(), "YYYYMMDD")
    ...
    ' Header
    Dim hdr As Variant
    hdr = Array("Scenario", "#", "Task", "Plan Start", "Plan End", "Act Start", "Act End", _
                "Start Var(d)", "End Var(d)", "% Done", "Status")
    Dim col As Long
    For col = 0 To UBound(hdr)
        With wsV.Cells(1, col + 1)
            .Value = hdr(col)
            .Font.Bold = True
            .Font.Size = 9
    ...
    For Each scenario In Array("BASE", "BEST", "WORST")
        ...
        For gR = FIRST_DATA_ROW To gLast
            ...
            Dim planS As Date, planE As Date
            Dim actS As Variant, actE As Variant
            Dim varS As Long, varE As Long
            Dim pct As Double, status As String
            ...
            ' Status
            If pct >= 1 Then
                status = IIf(varE <= 0, "DONE ON TIME", "DONE LATE")
            ElseIf pct > 0 Then
                status = "IN PROGRESS"
            ...
            wsV.Cells(row, 1).Value = CStr(scenario)
            wsV.Cells(row, 2).Value = wsG.Cells(gR, COL_STEP).Value
            wsV.Cells(row, 3).Value = wsG.Cells(gR, COL_TASK).Value
            wsV.Cells(row, 4).Value = Format(planS, "yyyy-mm-dd")
            wsV.Cells(row, 5).Value = Format(planE, "yyyy-mm-dd")
            wsV.Cells(row, 6).Value = IIf(IsDate(actS), Format(CDate(actS), "yyyy-mm-dd"), "")
            wsV.Cells(row, 7).Value = IIf(IsDate(actE), Format(CDate(actE), "yyyy-mm-dd"), "")
            wsV.Cells(row, 8).Value = varS
            wsV.Cells(row, 9).Value = varE
            wsV.Cells(row, 10).Value = Format(pct, "0%")
            wsV.Cells(row, 11).Value = status

            ' Conditional colour on status
            Select Case status
                Case "DONE ON TIME"
                    wsV.Cells(row, 11).Interior.Color = HexToLong(C_GREEN)
                Case "DONE LATE", "OVERDUE"
                    wsV.Cells(row, 11).Interior.Color = HexToLong(C_RED)
                Case "IN PROGRESS", "STARTED"
                    wsV.Cells(row, 11).Interior.Color = HexToLong(C_AMBER)
            End Select

            ' Colour variance columns
            If Abs(varS) > 0 Then wsV.Cells(row, 8).Font.Color = HexToLong(IIf(varS > 0, C_RED, C_GREEN))
            If Abs(varE) > 0 Then wsV.Cells(row, 9).Font.Color = HexToLong(IIf(varE > 0, C_RED, C_GREEN))
...
NextVarScen:
    ...
    LogMsg "Variance_Report", rName & " created"
    MsgBox "Variance Report: " & rName, vbInformation, "Variance"
    Exit Sub
EH:
    AppOff
