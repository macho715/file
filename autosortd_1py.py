...
def trim_cache(cache: Dict[str, Any], max_entries: int = MAX_CACHE_ENTRIES) -> Dict[str, Any]:
    if len(cache) <= max_entries:
        return cache
    sorted_keys = sorted(cache, key=lambda k: str(cache[k].get("run_id", "")))
    to_remove = sorted_keys[: len(cache) - max_entries]
    for k in to_remove:
        cache.pop(k, None)
    return cache
...
def wait_until_stable(p: Path, timeout_s: int = 20) -> bool:
    ...
    while time.time() - t0 < timeout_s:
        try:
            ...
            return False
        if sz == last and (sz > 0 or (last == 0 and stable_hits > 0)):
            stable_hits += 1
        else:
            stable_hits = 0
        if stable_hits >= 2:
    ...
    for r in rules_cfg.get("rules", []) or []:
        ...
        if rtype == "ext":
            ...
            try:
                ...
                print(
                    f"[WARN] Rule '{rid}': invalid regex '{pat}': {e}",
                    file=sys.stderr,
                )
                continue
        else:
...
def move_with_ledger(
    ...
    if dst.exists():
        ...
        for i in range(1, 10000):
            cand = dst_dir / f"{base}__{i:05d}{dst.suffix}"
            if not cand.exists():
                dst = cand
                break
        else:
            dst = dst_dir / f"{base}__{sha[:8]}{dst.suffix}"
...
def check_quarantine_count(paths: Paths) -> None:
    try:
        if not paths.quarantine.exists():
            return
        files = [p for p in paths.quarantine.iterdir() if p.is_file()]
        if len(files) > QUARANTINE_ALERT_THRESHOLD:
            print(
                f"[ALERT] Quarantine has {len(files)} files "
                f"(threshold: {QUARANTINE_ALERT_THRESHOLD})"
            )
    except Exception:
        pass
...
def main():
    ...
    ap.add_argument(
        "--no-llm",
        action="store_true",
        help="skip LLM; route ambiguous docs to quarantine",
    )
    args = ap.parse_args()
    ...
    try:
        last_quarantine_check = time.time()
        while True:
            time.sleep(1)
            if time.time() - last_quarantine_check >= QUARANTINE_CHECK_SECONDS:
                check_quarantine_count(paths)
                last_quarantine_check = time.time()
