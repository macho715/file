...
Private Sub DrawLegend(ByVal ws As Worksheet)
    ' ── Legend : Row 2 전체를 merge하여 한 줄로 표시 ──────────
    ' 형식: LEGEND:  [■ Doc/Pickup]  [■ UAE Repair] ...
    ' ■ 기호는 Chr(9632), 배경색은 각 Phase 색으로 개별 셀 칩 사용
    On Error GoTo EH

    Const LEG_ROW  As Long = 2
    Const CHIP_SZ  As Long = 2   ' 칩 셀 병합 폭 (열 수)

    Dim lastC As Long: lastC = HeaderLastCol(ws)
    If lastC < DATE_COL_START Then lastC = DATE_COL_START + 30
    ...
    ws.Rows(LEG_ROW).RowHeight = 17
    With ws.Range(ws.Cells(LEG_ROW, 1), ws.Cells(LEG_ROW, lastC))
    ...
    ' ── 항목 데이터 정의 ──────────────────────────────────────
    '  col 1  : "LEGEND:"  레이블
    '  col 3,5,7,9,11,13,15 : 컬러 칩 (2열 merge)
    '  col 4,6,8,10,12,14,16: 텍스트 레이블
    ' 칩 시작 컬럼
    Dim chipCols(6) As Long
    chipCols(0) = 2   ' B  Blue
    chipCols(1) = 4   ' D  Purple
    chipCols(2) = 6   ' F  Orange
    chipCols(3) = 8   ' H  Pink
    chipCols(4) = 10  ' J  Green
    chipCols(5) = 12  ' L  Yellow
    chipCols(6) = 14  ' N  Red  ← date track 영역 시작부와 겹치지 않게 meta 내

    Dim chipColors(6) As String
    chipColors(0) = C_BLUE
    chipColors(1) = C_PURPLE
    chipColors(2) = C_ORANGE
    chipColors(3) = C_PINK
    chipColors(4) = C_GREEN
    chipColors(5) = C_YELLOW
    chipColors(6) = C_RED

    Dim chipLabels(6) As String
    chipLabels(0) = "Doc / Pickup"
    chipLabels(1) = "UAE Repair"
    chipLabels(2) = "Transit OUT"
    chipLabels(3) = "KSA Repair"
    chipLabels(4) = "Transit BACK"
    chipLabels(5) = "Final"
    chipLabels(6) = "HIGH Risk"
    ...
    ' ── 각 항목: 칩 셀 + 텍스트 셀 ────────────────────────────
    Dim idx As Long
    Dim cc  As Long   ' chip column
    Dim tc  As Long   ' text column
    Dim fgColor As String

    For idx = 0 To 6
        cc = chipCols(idx)      ' 칩 컬럼
        tc = chipCols(idx) + 1  ' 텍스트 컬럼

        ' 칩 셀
        With ws.Cells(LEG_ROW, cc)
            .Interior.Color      = HexToLong(chipColors(idx))
            .Value               = Chr(9632)   ' ■
            .Font.Size           = 9
            .Font.Bold           = True
            .HorizontalAlignment = xlCenter
            .VerticalAlignment   = xlCenter
        End With
        ws.Columns(cc).ColumnWidth = 2.5

        ' 텍스트 셀 — 배경은 Phase 색 투명감, 글씨는 대비색
        Select Case chipColors(idx)
            Case C_YELLOW, C_GREEN:  fgColor = "0D0F14"   ' 어두운 글씨
            Case Else:               fgColor = "EEFCFF"   ' 밝은 글씨
        End Select

        With ws.Cells(LEG_ROW, tc)
            .Value               = chipLabels(idx)
            .Font.Size           = 7
            .Font.Bold           = True
            .Font.Color          = HexToLong(fgColor)
            .Interior.Color      = HexToLong(chipColors(idx))
            .HorizontalAlignment = xlLeft
            .VerticalAlignment   = xlCenter
            .WrapText            = False
        End With
        ws.Columns(tc).ColumnWidth = 9.5

        ' 칩·텍스트 셀 테두리
        SetThinBorder ws.Cells(LEG_ROW, cc)
        SetThinBorder ws.Cells(LEG_ROW, tc)
    Next idx

    ' ── 날짜 트랙 영역(N~ = DATE_COL_START~)은 BG_HEADER로 채움 ─
    If DATE_COL_START <= lastC Then
        With ws.Range(ws.Cells(LEG_ROW, DATE_COL_START), ws.Cells(LEG_ROW, lastC))
            .Interior.Color = HexToLong(BG_HEADER)
            .ClearContents
        End With
    End If
